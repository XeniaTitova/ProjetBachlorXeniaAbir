...
for img in tout_image:
    ...
    try:
        #pos_centre, pos_coin = modul_aruco.find_pos_3d_to_2d(nom_im = "image" +"/" + img, nom_out_aruco = nom_out_aruco, 
         #   nom_out_axe = nom_out_axe, nom_out_detect = nom_out_detect, obj_bleu = pts_centre , obj_vert = pts_coin)
        
        markerCorners, markerIds = modul_aruco.detect_aruco(nom_im = "image" +"/" + img, nom_out = nom_out_aruco)
        W = 1
        id_origine = 0
        while(id_origine < 10 and W == 1):
            F = 0
            ok = 0
            try:
                pos_centre, pos_coin = modul_aruco.find_pos_3d_to_2d_2(markerCorners, markerIds, id_origine, nom_im = "image" +"/" + img, 
                    nom_out_axe = nom_out_axe, nom_out_detect = nom_out_detect, obj_bleu = pts_centre , obj_vert = pts_coin)

                #if(len(pos_coin)):
                pos_coint_reel,W,I = modul_aruco.trouve_contour(pos_coin, name_img = "image" +"/" + img, name_out = nom_out_img_ok, h = 25, w = 25, aire_min = 30,  aire_max = 60)
                ok = 1
